...
class Camera2BasicFragment : Fragment(), FragmentCompat.OnRequestPermissionsResultCallback {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fabTakePicture.setOnClickListener {
            val bitmap = textureView.getBitmap(ImageClassifier.DIM_IMG_SIZE_X, ImageClassifier.DIM_IMG_SIZE_X)
            backgroundHandler?.post {
                val file = File(activity.getExternalFilesDir(Environment.DIRECTORY_PICTURES),
                        "picture" + System.currentTimeMillis() + ".jpg")
                var os: OutputStream? = null
                try {
                    val result = classifier?.classifyFrame(bitmap) ?: emptyList()
                    os = FileOutputStream(file)
                    bitmap.compress(Bitmap.CompressFormat.PNG, 100, os)
                    os.flush()
                    os.close()
                    if (result.isNotEmpty()) {
                        val diagnosis = Diagnosis(result.first().first, (result.first().second * 100).toInt(), file.path)
                        activity.runOnUiThread {
                            openDetailActivity(diagnosis)
                        }
                    }
                } catch (e: IOException) {
    ...
    private fun openDetailActivity(diagnosis: Diagnosis) {
        startActivity(Intent(activity, DetailActivity::class.java).apply {
            putExtra(DetailActivity.DIAGNOSIS, diagnosis)
        })
    }
    ...
    private fun classifyFrame() {
        ...
        val bitmap = textureView.getBitmap(ImageClassifier.DIM_IMG_SIZE_X, ImageClassifier.DIM_IMG_SIZE_Y)
        val textToShow = classifier!!.classifyFrame(bitmap)
        if (textToShow.size > 0) {
            Log.d(Camera2BasicFragment.TAG, textToShow[0].first)
            showToast(textToShow[0].first + ": " + textToShow[0].second)
        }
        bitmap.recycle()
    }
